...
void timerFlagProc(void)
{
	...
	asm {
			/* The time manager puts the address */
...
{
	move.l A5, A0 move.l struct(TMInfo_s.tmRefCon)(A1), A5 move.w #1, gAnimExpired move.l A0, A5 move.l A1, A0 move.l #TIME_PER_STEP, D0 _PrimeTime rts
}
#endif
...
#ifdef __MWERKS__
	if (nrows)
		SIOUXSettings.rows = nrows;
	if (ncols)
		SIOUXSettings.columns = ncols;
...
#else
	if (nrows)
		console_options.nrows = nrows;
	if (ncols)
		console_options.ncols = ncols;
#endif
	if (console_flag)
	{ /* don't open console unnecessarily */
...
extern int GP142_open(void)
{
	...
	if (!(size_hint = XAllocSizeHints()) || !(wm_hint = XAllocWMHints()) || !(class_hint = XAllocClassHint()) || (display = XOpenDisplay(display_name)) == NULL)
	{
	...
	XSelectInput(display, window, StructureNotifyMask | ExposureMask | KeyPressMask | ButtonPressMask | EnterWindowMask | LeaveWindowMask);
	XSelectInput(display, wincomm, StructureNotifyMask | ExposureMask | KeyPressMask | ButtonPressMask | EnterWindowMask | LeaveWindowMask);

	font_info = (FontDB *)malloc(sizeof(FontDB));
	if ((font_info == NULL) ||
			(font_info->thefont = XLoadQueryFont(display,
																					 "-adobe-times-medium-r-normal--0-150-75-75-p-0-iso8859-1")) == NULL)
	{
	...
	for (i = 0; i < 8; i++)
	{
		winc[i] = XCreateSimpleWindow(display, wincomm, (i / 4) * 100,
																	((i % 4) * 25) + 5, 95, 20, 2, BlackPixel(display, screen), WhitePixel(display, screen));
		XSelectInput(display, winc[i], StructureNotifyMask | ExposureMask | KeyPressMask | ButtonPressMask | EnterWindowMask | LeaveWindowMask);
...
/******************************************************* GP142_circle() ***/
extern int GP142_FUNC
GP142_circleXY(int color, int x, int y, int radius, int width)
{
	GP142_point p;
	...
	return GP142_circleP(color, p, radius, width);
}

extern int GP142_FUNC
GP142_circleP(int color, GP142_point center, int radius, int width)
{
	Graphics new_item;
...
static void GP_clear(void)
{
	...
	HBRUSH hBrush, hOldBrush;
	hBrush = CreateSolidBrush(gRGBColors[0]);
	hOldBrush = SelectObject(offscreen_DC, hBrush);
	...
	case obj_rectangle:
		...
		if (item->width == 0)
			XFillRectangle(display, offscreen_buffer, gc,
										 gXCenter + item->g.rectangle.upper_left.x,
										 gYCenter - item->g.rectangle.upper_left.y,
										 item->g.rectangle.lower_right.x - item->g.rectangle.upper_left.x + 1,
										 item->g.rectangle.upper_left.y - item->g.rectangle.lower_right.y + 1);
		else
			XDrawRectangle(display, offscreen_buffer, gc,
										 gXCenter + item->g.rectangle.upper_left.x,
										 gYCenter - item->g.rectangle.upper_left.y,
										 item->g.rectangle.lower_right.x - item->g.rectangle.upper_left.x,
										 item->g.rectangle.upper_left.y - item->g.rectangle.lower_right.y);
		break;
	...
	case obj_oval:
		...
		if (item->width == 0)
			XFillArc(display, offscreen_buffer, gc,
							 gXCenter + item->g.oval.upper_left.x,
							 gYCenter - item->g.oval.upper_left.y,
							 item->g.oval.lower_right.x - item->g.oval.upper_left.x,
							 item->g.oval.upper_left.y - item->g.oval.lower_right.y,
							 0, 23360);
		else
...
{
	return GetPixel(offscreen_DC, gXCenter + x, gYCenter - y);
}

COLORREF get_color_colorref(int color)
{
	return gRGBColors[color];
}

void set_Pixel(int x, int y, COLORREF c)
{
	SetPixel(offscreen_DC, gXCenter + x, gYCenter - y, c);
}
